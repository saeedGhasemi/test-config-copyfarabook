-- Backfill: convert bare URLs in legacy paragraph/quote/callout/heading text
-- into markdown links [url](url) so the reader displays them as clickable
-- links (matches new word-import behavior).
DO $$
DECLARE
  rec RECORD;
  new_pages JSONB;
  ...
  new_txt TEXT;
  btype TEXT;
  url_re CONSTANT TEXT := '(?<![\(\[])\b(https?://[^\s<>"''\)]+)';
BEGIN
    ...
    FOR page_idx IN 0 .. jsonb_array_length(new_pages) - 1 LOOP
      ...
      FOR block_idx IN 0 .. jsonb_array_length(blocks) - 1 LOOP
        ...
        IF btype IN ('paragraph','quote','callout','heading','highlight') THEN
          txt := block_obj ->> 'text';
          IF txt IS NOT NULL AND txt ~ 'https?://' THEN
            -- Skip strings that already contain markdown links
            IF txt !~ '\]\(https?://' THEN
